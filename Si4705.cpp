...
********************************************************/
void Si4705::initRadio(int Power, int volume, int resetPin, int intPin, int auxPin, unsigned FMchannel)
{
	Wire.begin();
...
*-----------------FM Channel Filter--------------------*
*******************************************************/
void Si4705::setChFilter (int channelFilter)
{
	Wire.beginTransmission(Si4705_Addr);
	...
	Wire.write(channelFilter >> 8);
	Wire.write(channelFilter&0xFF);
	Wire.endTransmission();
	delay(10);
}
...
*-------------------FM Sendersuchlauf------------------* 
********************************************************/
/* unsigned Si4705::seekStation (int Direction)
{
...
void Si4705::decodeRT (void)
{
  ...
  int strl      = 0;
  int BLER		= RDS[12];
  if((GroupType==2)&&(RT_decodeProgress<2)&&(RTflag==false)&&(RDSSynch))
  {
      RTAdress=RDS[7]&15;
      RTAdress*=4;
      ...
      if((RT_decodeProgress==1)&&(strl!=0)&&(RTAdress==0))
      {
          ...
          for(int copy = LZA; (copy <= LZE); copy++)RT[copy] = RTTemp[copy];
          for(int Clear=0; Clear<=64; Clear++)      RTTemp[Clear]= 0;
          RT_decodeProgress=0;
          RTflag = true;
      }
...
void Si4705::decodePTY (void)
{
  ...
  switch(_PTY)
  {
    ...
    case 11:strcpy(PTY,"Rock Musk\0");break;
    case 12:strcpy(PTY,"Easy List\0");break;
...
void Si4705::decodeTC (void)
{
  ...
  if((GroupType==0)&&(RDSSynch))
  {
	  ...
	  if(((RDS[6]&(1<<2)))&&((RDS[7]&(1<<4))==0)) 		TC=2;	// TP Signal is available
	  if(((RDS[6]&(1<<2)))&&((RDS[7]&(1<<4))))  		TC=3;	// Traffic announcement is now
  }
}
...
void Si4705::decodeAF (void)
{
	...
	{
		_Puffer0=((RDS[8]+875)*10);
		_Puffer1=((RDS[9]+875)*10);
